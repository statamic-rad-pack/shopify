...
Allows you to filter the Products Collection to items that have at least one variant in stock. A variant counts as in stock when its inventory quantity is above zero, or when its inventory policy allows overselling (i.e. it isn't set to `deny`).

#### Usage

```twig
{{ collection:products query_scope="product_has_stock" as="products" }}
