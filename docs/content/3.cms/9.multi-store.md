...
'multi_store' => [
    ...
    'stores' => [
        'uk' => [
            ...
            'webhook_secret' => env('SHOPIFY_STORE_UK_WEBHOOK_SECRET'), // = client_secret for Admin API–registered webhooks
        ...
        'us' => [
            ...
            'webhook_secret' => env('SHOPIFY_STORE_US_WEBHOOK_SECRET'), // = client_secret for Admin API–registered webhooks
...
When a store's webhooks are registered via the Artisan command / Admin API, its `webhook_secret` must be set to that store's Client Secret (`client_secret`), not the secret from **Settings > Notifications**. See [Webhooks](/cms/webhooks) for details.
