...
Each store's webhook secret must be set individually. See [Env Values](/installation/env) for the per-store env var pattern (`SHOPIFY_STORE_{HANDLE}_WEBHOOK_SECRET`).
