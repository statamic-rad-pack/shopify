...
| Value             | Description  |
| -------------------| ------------- |
| `SHOPIFY_WEBHOOK_SECRET`  | The secret used to verify incoming webhook signatures. For webhooks registered via the Artisan command / Admin API (the recommended path) this is your app's Client Secret — set it to the same value as `SHOPIFY_CLIENT_SECRET`. Read more about this [here](/cms/webhooks)  |
...
| `SHOPIFY_STORE_UK_WEBHOOK_SECRET` | Webhook signing secret for this store. For Admin API–registered webhooks this is the store's Client Secret (`SHOPIFY_STORE_UK_CLIENT_SECRET`). |
