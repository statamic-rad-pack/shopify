...
Customer, address and order data is read through the Shopify Admin API, which is not scoped to a single customer — the token can see the whole store. The customer tags default to the logged in user, but any code that fetches a customer or renders an order by an ID taken from the request (a route parameter, query string, form field) must confirm that ID belongs to the logged in user first. Compare `order.customer.id` to the current user's `shopify_id` before displaying it. This fails silently — the wrong order renders without an error.
