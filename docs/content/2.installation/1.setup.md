...
If you are not intending to use the Shopify hosted storefront you should perform redirection from any Shopify URLs to your website domain. 
...
SHOPIFY_WEBHOOK_SECRET="{App Client Secret}"
```

::note
When you register webhooks with `php artisan shopify:webhooks:register` (step 4), Shopify signs them with your app's Client Secret, so `SHOPIFY_WEBHOOK_SECRET` should be set to the same value as `SHOPIFY_CLIENT_SECRET`. Only use the secret from **Settings > Notifications** if you create webhooks manually there.
::

You may also wish to add some of the optional values defined in the ["Env Values"](/installation/env) section.
