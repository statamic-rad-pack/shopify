...
6. Click the "API Credentials" tab. Add the `Admin API access token` to your `.env` as `SHOPIFY_AUTH_PASSWORD`, add `API key` as `SHOPIFY_AUTH_KEY`, add `API secret key` as `SHOPIFY_WEBHOOK_SECRET` and add `Storefront API access token` as `SHOPIFY_STOREFRONT_TOKEN`.
